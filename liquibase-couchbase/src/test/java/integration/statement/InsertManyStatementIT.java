...
class InsertManyStatementIT extends TransactionStatementTest {
    private final List<Document> testDocuments = ImmutableList.of(new Document(TEST_ID, TEST_DOCUMENT.toString()),
    ...
    void Should_no_insert_documents_when_transaction_was_broken() {
        ...
        assertThatExceptionOfType(TransactionFailedException.class).isThrownBy(
                () -> doInFailingTransaction(statement.asTransactionAction(clusterOperator)));
