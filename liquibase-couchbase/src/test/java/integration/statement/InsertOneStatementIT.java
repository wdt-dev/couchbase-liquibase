...
class InsertOneStatementIT extends TransactionStatementTest {
    ...
    public void setUp() {
        ...
        defaultCollectionOperator = new CollectionOperator(
                bucketOperator.getCollectionFromDefaultScope(DEFAULT_COLLECTION));
    }

    @Test
    void Should_insert_one_document() {
        InsertOneStatement statement = new InsertOneStatement(TEST_KEYSPACE,
    ...
    void Should_insert_document_to_default_scope() {
        ...
        ReactiveCollection collection = bucketOperator.getCollectionFromDefaultScope(TEST_COLLECTION_2);
        assertThat(collection).hasDocument(TEST_ID);
    ...
    void Should_no_insert_documents_when_transaction_was_broken() {
        ...
        InsertOneStatement statement1 = new InsertOneStatement(keyspace,
                document(uncreatedKey1, TEST_DOCUMENT.toString()));
        InsertOneStatement statement2 = new InsertOneStatement(keyspace,
                document(uncreatedKey2, TEST_DOCUMENT.toString()));
        InsertOneStatement statement3 = new InsertOneStatement(keyspace,
                document(existingKey, TEST_DOCUMENT.toString()));

        assertThatExceptionOfType(TransactionFailedException.class).isThrownBy(
                () -> doInTransaction(statement1.asTransactionAction(clusterOperator),
                        statement2.asTransactionAction(clusterOperator),
                        statement3.asTransactionAction(clusterOperator)));

        ReactiveCollection collection = bucketOperator.getCollectionFromDefaultScope(DEFAULT_COLLECTION);
        assertThat(collection).hasDocument(existingKey).hasNoDocument(uncreatedKey1).hasNoDocument(uncreatedKey2);
