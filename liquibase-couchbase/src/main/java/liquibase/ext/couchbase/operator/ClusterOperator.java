...
/**
 * A part of a facade package for Couchbase Java SDK. Provides access to {@link Cluster} common operations and state
 * checks.
 * @see BucketOperator
 */
...
public class ClusterOperator {
    ...
    protected final ReactiveCluster cluster;
    ...
    public void createQueryIndex(String indexName, String bucket, List<Field> fields, CreateQueryIndexOptions options) {
        List<String> fieldList = fields.stream().map(Field::getField).collect(toList());
    ...
    public ReactiveQueryIndexManager getQueryIndexes() {
        return cluster.queryIndexes();
    }

    public Flux<QueryIndex> getQueryIndexesForBucket(String bucketName) {
    ...
    public void createIndex(String name, Keyspace keyspace, List<String> fieldList) {
        ...
        CreateQueryIndexOptions options = CreateQueryIndexOptions.createQueryIndexOptions().scopeName(
                keyspace.getScope()).collectionName(keyspace.getCollection());
    ...
    public void dropIndex(String indexName, Keyspace keyspace) {
        DropQueryIndexOptions options = DropQueryIndexOptions.dropQueryIndexOptions().collectionName(
                keyspace.getCollection()).scopeName(keyspace.getScope());
    ...
    public boolean indexExists(String indexName, Keyspace keyspace) {
        GetAllQueryIndexesOptions options = GetAllQueryIndexesOptions.getAllQueryIndexesOptions().collectionName(
                keyspace.getCollection()).scopeName(keyspace.getScope());
        return getQueryIndexes().getAllIndexes(keyspace.getBucket(), options).toStream().map(QueryIndex::name).anyMatch(
                indexName::equals);
    }

    public void dropPrimaryIndex(Keyspace keyspace) {
        DropPrimaryQueryIndexOptions options = DropPrimaryQueryIndexOptions.dropPrimaryQueryIndexOptions()
    ...
    public Map<String, JsonObject> checkDocsAndTransformToJsons(List<Document> documents) {
        try {
            return documents.stream().collect(toMap(Document::getId, ee -> JsonObject.fromJson(ee.getContent())));
        }
