...
public abstract class CouchbaseFileContentStatement extends CouchbaseTransactionStatement {
    ...
    private final Map<ImportType, DocFileMapper> importTypeToProcessor = ImmutableMap.of(
            ImportType.LINES, new LinesMapper(),
            ImportType.LIST, new ListMapper()
    );

    protected Map<String, Object> importFromFileWith(File file,
                                                     ClusterOperator clusterOperator) {

        val documents = importTypeToProcessor.get(file.getImportType()).map(file);
        return clusterOperator.checkDocsAndTransformToObjects(documents);
    }
}
